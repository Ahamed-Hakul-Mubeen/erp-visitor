...
<head>
    ...
    <style type="text/css">
        ...
        * {
        ...
        .pre_order-summary td,
        .pre_order-summary th {
            font-size: 13px;
            font-weight: 600;
        }
...
<body>
<div class="pre_order-preview-main" id="boxes">
    <div class="pre_order-header" style="border-top: 15px solid var(--theme-color); background: #f8f8f8;">
        <table>
            <tbody>
            <tr>
                <td>
                    <img class="pre_order-logo"
                         src="{{$img}}"
                         alt="">
                </td>
                <td class="text-right">
                    <h3 style="text-transform: uppercase; font-size: 40px; font-weight: bold;">{{__('Pre Order')}}</h3>
                </td>
            </tr>
            </tbody>
        </table>
            ...
            <tr>
                ...
                <td>
                    <table class="no-space"  style="width: 45%;margin-left: auto;">
                        ...
                        <tr >
                            <td>{{__('Number')}}:</td>
                            <td class="text-right">{{Utility::preOrderNumberFormat($settings,$pre_order->pre_order_id)}}</td>
                        </tr>
                        <tr>
                            <td>{{__('Issue Date')}}:</td>
                            <td class="text-right">{{Utility::dateFormat($settings,$pre_order->issue_date)}}</td>
                        </tr>
                        ...
                        @if(!empty($customFields) && count($pre_order->customField)>0)
                            @foreach($customFields as $field)
                                <tr>
                                    <td>{{$field->name}} :</td>
                                    <td> {{!empty($pre_order->customField)?$pre_order->customField[$field->id]:'-'}}</td>
                                </tr>
                            @endforeach
                        @endif
                        <tr>
                            <td colspan="2">
                                <div class="view-qrcode">
                                    {!! DNS2D::getBarcodeHTML(route('pre_order.link.copy',\Crypt::encrypt($pre_order->pre_order_id)), "QRCODE",2,2) !!}
                                </div>
                            </td>
                        </tr>
                        </tbody>
    ...
    <div class="pre_order-body" style="border-bottom: 15px solid var(--theme-color);">
        <table class="vertical-align-top">
            <tbody>
            <tr>
        ...
        <table class="add-border pre_order-summary" style="margin-top: 30px;">
            ...
            <tbody>
            @if(isset($pre_order->itemData) && count($pre_order->itemData) > 0)
                @foreach($pre_order->itemData as $key => $item)
                    <tr >
                        <td>{{$item->name}}</td>
                        @php
            ...
            <tr>
                <td>{{__('Total')}}</td>
                <td>{{$pre_order->totalQuantity}}</td>
                <td>{{Utility::priceFormat($settings,$pre_order->totalRate)}}</td>
                <td>{{Utility::priceFormat($settings,$pre_order->totalDiscount)}}</td>
                <td>{{Utility::priceFormat($settings,$pre_order->totalTaxPrice) }}</td>
                <td>{{Utility::priceFormat($settings,$pre_order->getSubTotal())}}</td>
            </tr>
            <tr>
                <td colspan="4"></td>
                <td colspan="2" class="sub-total">
                    <table class="total-table">
                        ...
                        <tr>
                            <td>{{__('Discount')}}:</td>
                            <td>{{Utility::priceFormat($settings,$pre_order->getTotalDiscount())}}</td>
                        </tr>
                        @if(!empty($pre_order->taxesData))
                            @foreach($pre_order->taxesData as $taxName => $taxPrice)
                                <tr >
                                    <td>{{$taxName}} :</td>
                        ...
                        <tr>
                            <td>{{__('Total')}}:</td>
                            <td>{{Utility::priceFormat($settings,$pre_order->getSubTotal()-$pre_order->getTotalDiscount()+$pre_order->getTotalTax())}}</td>
                        </tr>
