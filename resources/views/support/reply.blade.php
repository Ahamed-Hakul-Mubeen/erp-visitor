...
@section('title')
    <div class="d-inline-block">
        <h5 class="mb-0 h4 d-inline-block font-weight-400 ">{{__('Support Reply')}}</h5>
    </div>
@endsection
@section('breadcrumb')
                            ...
                            <div class="p-0 card-body">
                                <div class="p-4 border-bottom">
                                    @if($support->priority == 0)
                                        <span class="mb-2 badge bg-primary">   {{ __(\App\Models\Support::$priority[$support->priority]) }}</span>
                                    @elseif($support->priority == 1)
                                        <span class="mb-2 badge bg-info">   {{ __(\App\Models\Support::$priority[$support->priority]) }}</span>
                                    @elseif($support->priority == 2)
                                        <span class="mb-2 badge bg-warning">   {{ __(\App\Models\Support::$priority[$support->priority]) }}</span>
                                    @elseif($support->priority == 3)
                                        <span class="mb-2 badge bg-danger">   {{ __(\App\Models\Support::$priority[$support->priority]) }}</span>
                                    @endif
                                    <div class="d-flex justify-content-between align-items-center ">
                                        <h5>{{$support->subject}}</h5>
                                        @if($support->status == 'Open')
                                            <span class="p-2 rounded badge bg-light-primary f-w-600 text-primary"> {{__('Open')}}</span>
                                        @elseif($support->status == 'Close')
                                            <span class="p-2 rounded badge bg-light-danger f-w-600 text-danger">   {{ __('Closed') }}</span>
                                        @elseif($support->status == 'On Hold')
                                            <span class="p-2 rounded badge bg-light-warning f-w-600 text-warning">   {{ __('On Hold') }}</span>
                                        @endif
                                    </div>
                                    <p class="mb-0">
                                ...
                                @if(!empty($support->description))
                                    <div class="p-4">
                                        ...
                                        <a href="{{asset(Storage::url('uploads/supports')).'/'.$support->attachment}}"
                                           download="" class="p-2 text-white rounded bg-secondary d-inline-flex " target="_blank">
                                            <i class="mt-1 text-white ti ti-download me-2" data-bs-toggle="tooltip" ></i>
                            ...
                            <div class="card-body">
                               <div class="row ">
                                   ...
                                   <div class="col-6 text-end">
                                       <a href="#" data-size="md" class="mb-3 btn btn-primary btn-icon btn-sm me-2" data-ajax-popup-over="true" id="grammarCheck" data-url="{{ route('grammar',['grammar']) }}"
                                ...
                                <textarea class="mb-2 form-control form-control-light grammer_textarea" name="description" placeholder="Your comment" id="example-textarea" rows="3" required=""></textarea>
                                <div class="text-end">
                                    <div class="col-12">
                                        <button type="submit" class="btn btn-primary w-100"> <i class="mb-0 ti ti-circle-plus me-1"></i> {{__('Send')}}</button>
                                    </div>
                                </div>
                                {{ Form::close() }}
                            </div>
                    ...
                    @foreach($replyes as $reply)
                        <div class="border card">
                            <div class="card-header row d-flex align-items-center justify-content-between">
                                ...
                                <p class="col-auto mb-0 ms-1"> <span class="text-muted">{{$reply->created_at->diffForHumans()}}</span></p>
                            </div>
                            <div class="card-body">
