        ...
        if (\Auth::user()->can('bill report')) {
            ...
            foreach ($types as $type) {
                ...
                foreach ($subTypes as $subType) {
                    ...
                    foreach ($accounts as $account) {
                        $Balance = $account['totalCredit'] - $account['totalDebit'];
                        // if($account['code'] == 2100) // New Account Payable = Purchased - Account Payable
                        // {
                        //     $purchase_acc_code = ChartOfAccount::where('code', 2100)->where('created_by', \Auth::user()->creatorId())->first();

                        //     // Get Purchased Value
                        //     $purchase_amount = TransactionLines::selectRaw("SUM(debit) as purchase_amount")
                        //     ->where('created_by', \Auth::user()->creatorId())
                        //     ->where('date', '>=', $start)
                        //     ->where('date', '<=', $end)
                        //     ->where('account_id', $purchase_acc_code->id)
                        //     ->first();
                        //     // dd($purchase_amount->purchase_amount);
                        //     $Balance += $purchase_amount->purchase_amount;
                        // }
                        $totalBalance += $Balance;
                        if ($account['code'] == 2100 || $Balance != 0) {
                            $data['account_id'] = $account['id'];
                            $data['account_code'] = $account['code'];
                            $data['account_name'] = $account['name'];
                            $data['account'] = '';
            ...
            $net_income = 0; //  (Income - (Expenses + Costs of Goods Sold))

            $net_income_arr = ChartOfAccountType::where('created_by', \Auth::user()->creatorId())->whereIn('name', ['Income', 'Expenses', 'Costs of Goods Sold'])->get();
            foreach ($net_income_arr as $ni_arr) {
                $net_income_arr = TransactionLines::select('chart_of_accounts.name', \DB::raw('sum(debit) as totalDebit'), \DB::raw('sum(credit) as totalCredit'));
                $net_income_arr->leftjoin('chart_of_accounts', 'transaction_lines.account_id', 'chart_of_accounts.id');
                $net_income_arr->leftjoin('chart_of_account_types', 'chart_of_accounts.type', 'chart_of_account_types.id');
                $net_income_arr->where('chart_of_accounts.type', $ni_arr->id);
                $net_income_arr->where('transaction_lines.created_by', \Auth::user()->creatorId());
                $net_income_arr->where('transaction_lines.date', '>=', $start);
            ...
            if($net_income)
            {
                $earningsTotal['account_id'] = '';
                $earningsTotal['account_code'] = '';
                $earningsTotal['account'] = '';
                $earningsTotal['account_name'] = 'Net Income';
